...
let call_mutex = Promise.resolve();
function sleep(ms) {
...
async function ensureChatPage(browser) {
...
async function setTextareaValueAndSend(page, text) {
    ...
    await page.evaluate((txt) => {
        ...
        const editor = document.querySelector('textarea[aria-label="Message ChatGPT"], textarea, [contenteditable="true"]');
        if (editor) {
            editor.focus();
            ...
            if (!success || (editor.innerText || editor.value || '').length < txt.length / 2) {
    ...
    const result = await page.evaluate(() => {
        ...
        if (turns.length > 0) {
            ...
            if (content) {
                ...
                if (text.length > 10)
                    return { text, method: 'data-testid+content', debug };
            }
            const text = lastTurn.innerText || '';
            if (text.length > 10)
                return { text, method: 'data-testid', debug };
        }
        ...
        if (assistantNodes.length) {
            ...
            if (text.length > 10)
                return { text, method: 'author-role', debug };
        }
        // 3. Try generic prose/markdown
        const proseNodes = Array.from(document.querySelectorAll('.prose, .markdown, article'));
        debug.push(`prose: ${proseNodes.length}`);
        if (proseNodes.length) {
            const last = proseNodes[proseNodes.length - 1];
            const text = last.innerText || last.textContent || '';
            if (text.length > 10)
                return { text, method: 'prose', debug };
        }
        ...
        if (textDivs.length) {
            ...
            if (text.length > 10)
                return { text, method: 'text-div-fallback', debug };
        }
...
function answerLooksComplete(raw) {
    const trimmed = raw.trimEnd();
    if (!trimmed)
        return false;
    if (trimmed.endsWith(SENTINEL))
        return true;
    if (JSON_END_PATTERN.test(trimmed))
        return true;
    return false;
}
...
async function waitForStableAnswer(page) {
    let lastText = '';
    let stableCount = 0;
    const maxStable = 4;
...
async function sendQuestionAndGetAnswer(prompt) {
    ...
    while (await isGenerating(page) && busyAttempts < 10) {
        if (busyAttempts === 0)
            console.log('⏳ ChatGPT is busy, waiting...');
        await sleep(1000);
        busyAttempts++;
    }
