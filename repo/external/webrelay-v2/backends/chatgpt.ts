...
async function getLatestAssistantText(page: Page): Promise<string> {
    const result = await page.evaluate(() => {
        ...
        const textDivs = allDivs.filter(d => {
            ...
            // Exclude common cookie/consent/privacy patterns
            const isConsent = /cookie|datenschutz|privacy|zustimmen|agree|consent/i.test(text);
            return text.length > 50 && !text.includes('Message ChatGPT') && !isConsent;
        });
