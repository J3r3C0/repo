...
def run():
    ...
    # latest
    r = client.get("/api/why/latest", params={"log_path": str(tmp_log), "intent": "execute_action", "max_lines": 2000})
    assert r.status_code == 200, r.text
    data = r.json()
    assert data["ok"] is True
    assert data["event"].get("prompt") == "***REDACTED***"
    assert data["event"].get("body") == "***REDACTED***"
    assert isinstance(data["event"].get("raw_content"), str) and data["event"]["raw_content"].endswith("...(truncated)")
    ...
    # stats
    r = client.get("/api/why/stats", params={"log_path": str(tmp_log), "intent": "execute_action", "window_lines": 10000})
    assert r.status_code == 200, r.text
    data = r.json()
    assert data["ok"] is True
    assert data["stats"]["count"] == 1

    print("[OK] WHY-API smoke tests passed.")
