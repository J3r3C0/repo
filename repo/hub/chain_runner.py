...
from hub.job_chain_manager import JobChainManager
...
class ChainRunner:
    ...
    def __init__(
        ...
        chain_index_path = self.storage.DATA_DIR / "chain_index.json"
    ...
    def _process_spec(self, chain_id: str, spec: Dict[str, Any]):
        ...
        # --- DEPTH DERIVATION ---
        # Derive current depth from parent job info in ChainIndex (fallback to 0)
        parent_info = self.chain_manager.chain_index.get(parent_id) if parent_id else None
        parent_depth = (parent_info or {}).get("depth", 0)
        current_depth = parent_depth + 1
        
        # Update payload with derived depth
        payload["_chain_hint"]["depth"] = current_depth
        ...
        with get_db() as conn:
            ...
            # Register in ChainIndex (NEW: Critical for loop closure)
            # Use defensive registration
            self.chain_manager.chain_index.put(job_id, {
                "chain_id": chain_id,
                "role": "child",
                "root_job_id": spec["root_job_id"],
                "parent_llm_job_id": parent_id,
                "depth": current_depth
            })
