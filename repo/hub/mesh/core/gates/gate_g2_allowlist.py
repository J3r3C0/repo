...
def run_g2_allowlist_kind(job: Dict[str, Any], cfg: GateConfig) -> GateResult:
    """
    Check if job.kind is in the allowlist.
    Supports JobEnvelope v1 (action.kind) and legacy flattened jobs.
    """
    # 1. Extract kind canonically
    kind = job.get("kind")
    if not kind and "action" in job and isinstance(job["action"], dict):
        kind = job["action"].get("kind")
    
    # Fallback for deep structures (Phase 9/10)
    if not kind and "payload" in job and isinstance(job["payload"], dict):
        kind = job["payload"].get("kind") or job["payload"].get("task", {}).get("kind")
