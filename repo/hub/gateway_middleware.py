...
class GatewayEnforcer:
    ...
    def enforce(self, job_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # --- SCHEMA NORMALIZATION (JobEnvelope-lite) ---
        # The gate layer expects a small set of top-level fields (job_id, kind, provenance,
        # action, payload, ...). Our persisted Job model stores the executable kind under
        # payload.task.kind or payload.kind.
        #
        # To avoid drift ("G2 missing kind" / "kind only nested"), we generate a
        # canonical gate_input dict here. Downstream systems may still use richer job models.
        gate_input = self._coerce_job_for_gates(job_dict)
        
        # Run gate pipeline
        reports = run_gates_v1(gate_input, self.gate_config)
    ...
    def _coerce_job_for_gates(self, job_dict: Dict[str, Any]) -> Dict[str, Any]:
        """Create a stable, gate-facing job dict from various internal job shapes."""
        # job_id: gates use job_id (not "id") in evidence and patches
        job_id = job_dict.get("job_id") or job_dict.get("id")

        payload = job_dict.get("payload") if isinstance(job_dict.get("payload"), dict) else {}

        # Derive kind
        raw_kind = job_dict.get("kind") or payload.get("kind") or ""
        kind = raw_kind.strip() if isinstance(raw_kind, str) else str(raw_kind).strip()
        if not kind:
            task_obj = payload.get("task") if isinstance(payload.get("task"), dict) else {}
            kind = str(task_obj.get("kind") or "").strip()

        # Params are commonly nested either under payload.params or payload.task.params
        params = {}
        if isinstance(payload.get("params"), dict):
            params = payload.get("params")
        else:
            task_obj = payload.get("task") if isinstance(payload.get("task"), dict) else {}
            if isinstance(task_obj.get("params"), dict):
                params = task_obj.get("params")

        # Action object (JobEnvelope-style)
        action = job_dict.get("action") if isinstance(job_dict.get("action"), dict) else None
        if not action:
            action = {
                "kind": kind,
                "params": params,
            ...
            # Ensure action.kind exists for G2
            action = dict(action)
            if not action.get("kind"):
                action["kind"] = kind
            if "params" not in action and params:
                action["params"] = params

        coerced = dict(job_dict)
        coerced["job_id"] = job_id
        coerced["kind"] = kind
