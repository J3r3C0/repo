...
def make_app(node_id: str, auth_key: str = "shared-secret"):
    # Jailed Root for physical execution - mapped to project root
    ALLOWED_ROOT = _project_root.resolve()
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            # Calculate own endpoint (for now assuming 127.0.0.1 based on port)
            # In production, this would be an external IP or hostname
            host_ip = os.getenv("SHERATAN_HOST_IP", "127.0.0.1")
            port = os.getenv("SHERATAN_HOST_PORT", "8081") # default if not set in env
            endpoint = f"http://{host_ip}:{port}"
            
            hb = HeartbeatClient(core_url=core_url, host_id=node_id, endpoint=endpoint)
            await hb.start()
            app.state.hb = hb
    ...
    async def run_internal(req: Request):
        ...
        # Detect JobEnvelope v1
        is_envelope = (spec.get("schema_version") == "job_envelope_v1")
        
        kind = spec.get("kind")
        if is_envelope:
            kind = kind or spec.get("intent") or spec.get("action", {}).get("kind")
            args = spec.get("action", {}).get("params") or spec.get("params")
        else:
            kind = kind or spec.get("type")
            args = spec.get("params") or spec.get("args") or spec.get("metrics")
        ...
        if is_envelope:
            print(f"[{node_id}] DEBUG: Envelope-v1 detected")
        print(f"[{node_id}] DEBUG: args: {args}")
        ...
        async def _execute():
            if kind == "write_file":
                ...
                try:
                    rel_root = args.get("path") or args.get("root") or args.get("target") or "."
                    recursive = bool(args.get("recursive", False))
                    # Use a fresh resolve to ensure we are within project root
                    search_root = (ALLOWED_ROOT / rel_root).resolve()
                    if not str(search_root).startswith(str(ALLOWED_ROOT)):
                ...
                try:
                    rel_root = args.get("root") or args.get("path") or "."
                    ...
                    for root, dirs, files in os.walk(search_root):
                        p_root = Path(root)
                        current_depth = len(p_root.parts) - base_depth
                        ...
                        rel_p = p_root.relative_to(ALLOWED_ROOT)
                        results.append({
                            "path": str(rel_p),
                            "dirs": dirs,
                            "files": files,
                    ...
                    return {
                        "ok": True,
                        "attempt_id": attempt_id,
                        "result": {"tree": results, "root": str(rel_root), "count": len(results)}
                    }
                except Exception as e:
                    return {"ok": False, "error": str(e)}
