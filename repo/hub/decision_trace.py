...
# Track D: Robust path resolution for schemas in frozen state
SCHEMA_FILE = Path(config.BASE_DIR) / "schemas" / "decision_trace_v1.json"

if not SCHEMA_FILE.exists():
    # Attempt fallback to local schemas if BASE_DIR/schemas is missing (e.g. during build-time tests)
...
if SCHEMA_FILE.exists():
    ...
    # Minimal fallback logger if schema is entirely missing – prevents total system crash
