...
from hub.chain_index import ChainIndex
...
class JobChainManager:
    ...
    def resolve_chain_spec(
        ...
        *,
        ...
        from hub.database import get_db
        from hub import storage
        from hub.result_ref import safe_get, apply_transform
        from hub.template_resolver import (
    ...
    def on_job_complete(self, *, job_id: str, result: Dict[str, Any], conn=None) -> None:
        """
        Called by core when ANY job completes.
        If job_id is a child job in a chain:
          - store compact result
          - check total active activity in DB
          - if 0 active: dispatch_next_llm_step()
        """
        ...
        # Update chain context if we are finishing a spec (Phase 10)
        # We need to mark the spec as DONE in the DB
        from hub.database import get_db
        ...
        def _process_closure(c):
            ...
            if spec_id:
                self.storage.mark_chain_spec_done(c, chain_id, spec_id, ok=True)
            
            # Check for ANY remaining active activity (specs or jobs)
            active_count = self.storage.count_active_chain_activity(c, chain_id)
            
            if active_count == 0:
                if self.logger:
                    self.logger.info(f"[chain:{chain_id}] Loop closure: 0 active items. Triggering next LLM step.")
                self.save_chain(chain)
                
                try:
                    self.dispatch_next_llm_step(chain_id=chain_id)
                except Exception as e:
                    # Handle race condition (IntegrityError) or other dispatch failures
                    if "UNIQUE constraint failed" in str(e):
                        if self.logger:
                            self.logger.info(f"[chain:{chain_id}] Loop closure: Next turn already dispatched (idempotency hit).")
                    else:
                        if self.logger:
                            self.logger.error(f"[chain:{cid}] Failed to trigger next LLM step: {e}")
            else:
                if self.logger:
                    self.logger.info(f"[chain:{chain_id}] Loop stay: {active_count} items remaining.")
                self.save_chain(chain)

        if conn:
            _process_closure(conn)
        else:
            with get_db() as new_conn:
                _process_closure(new_conn)
    ...
    def dispatch_next_llm_step(self, *, chain_id: str, constraint_violation: Optional[str] = None) -> Optional[str]:
        ...
        # [Phase 10.2] Aggregate artifacts from DB context for rich prompt injection
        db_artifacts = {}
        try:
            from hub.database import get_db
            with get_db() as conn:
                ctx = self.storage.get_chain_context(conn, chain_id)
                if ctx and "artifacts" in ctx:
                    db_artifacts = ctx["artifacts"]
        except Exception as e:
            if self.logger:
                self.logger.warning(f"[chain:{chain_id}] Failed to load DB artifacts for prompt: {e}")
        ...
        llm_input = {
            ...
            "artifacts": db_artifacts, # Rich context from Phase 10
            "constraints": {
        ...
        job = Job(
            id=llm_job_id,
            task_id=root_job.task_id if root_job else chain["root_job_id"],
            payload=payload,
            status="pending",
            result=None,
            retry_count=0,
            idempotency_key=f"chain:{chain_id}:agent_plan:{next_depth}",
            priority="normal",
            timeout_seconds=300,
            depends_on=[],
            created_at=created,
        ...
        chain["last_tool_results"] = []  # Clear after consumption to prevent bloat
        self.save_chain(chain)
