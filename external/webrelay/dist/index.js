...
async function logLedgerEvent(event, jobId, zone, artifactPath, meta = {}) {
    try {
...
async function handleJob(job) {
    try {
        const data = await fs.readJSON(job.fullPath).catch(() => null);
        if (!data) {
...
async function writeResult(jobFilename, result) {
...
async function archiveJob(fullPath, filename, result) {
    try {
...
function debounceFile(fullPath) {
...
async function startFileWatcher() {
    ...
    const files = (await fs.readdir(RELAY_OUT)).filter((f) => f.endsWith('.json'));
    for (const f of files)
        debounceFile(path.join(RELAY_OUT, f));
...
async function registerWorker() {
    ...
    try {
        const response = await axios.post(`${CORE_URL}/api/mesh/workers/register`, registration, { timeout: 5000 });
