    ...
    try {
        ...
        if (parsed.action) {
            const lcpResult: any = {
                ...
                lcp_version: parsed.lcp_version || "1",
                ...parsed
            };

            // Normalize to LCP v1 type/jobs/answer if possible
            if (!lcpResult.lcp_type) {
                if (lcpResult.action === 'create_followup_jobs' || lcpResult.new_jobs?.length > 0) {
                    lcpResult.lcp_type = 'followup_jobs';
                    lcpResult.jobs = lcpResult.jobs || lcpResult.new_jobs;
                } else if (lcpResult.action === 'analysis_result' || lcpResult.answer) {
                    lcpResult.lcp_type = 'final_answer';
                    lcpResult.answer = lcpResult.answer || parsed;
                }
            }
            return lcpResult;
        }
