...
from hub.database import get_db, init_db
...
def count_active_chain_activity(conn, chain_id: str) -> int:
    """
    Counts active work items for a chain:
    1. Pending/Working specs in chain_specs.
    2. Non-finished jobs associated with this chain in jobs table.
    """
    # Count pending specs (not done/failed/dispatched)
    spec_count = conn.execute(
        "SELECT COUNT(*) FROM chain_specs WHERE chain_id=? AND status='pending'",
        (chain_id,)
    ).fetchone()[0]
    
    # Count jobs that are not completed/failed
    # Based on _chain_hint in payload for child jobs
