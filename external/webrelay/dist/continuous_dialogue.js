...
async function getActivePage(browser) {
    ...
    if (!pages.length)
        throw new Error('No pages found. Is Gemini open?');
...
async function getLatestResponse(page) {
    const text = await page.evaluate(() => {
        ...
        const last = nodes.filter(Boolean).slice(-1)[0];
        return last?.innerText || '';
    });
    return cleanText(text || '');
}
async function typeInto(page, content) {
...
async function sendWeiter(page) {
    await typeInto(page, 'weiter');
    await clickSend(page);
}
async function saveResponse(content, isInitial = false) {
