...
async function ensureGeminiPage(browser) {
...
async function setTextareaValueAndSend(page, text) {
    ...
    if (toInsert.length > 0) {
        ...
        await page.evaluate((txt) => {
            ...
            const editor = document.querySelector('div.ql-editor[contenteditable="true"], [contenteditable="true"], rich-textarea textarea');
            if (editor) {
                editor.focus();
...
async function isGenerating(page) {
    const found = await page.evaluate(() => {
        ...
        const indicators = Array.from(document.querySelectorAll('[class*="generating"], [class*="thinking"], [aria-label*="Stop"]'));
        return indicators.length > 0;
    });
    return !!found;
}
async function waitForStableAnswer(page, timeoutMs = 120_000) {
    let lastText = '';
    let stableCount = 0;
    const maxStable = 4;
    const pollMs = 1_000;
...
async function sendQuestionAndGetAnswer(prompt, config) {
